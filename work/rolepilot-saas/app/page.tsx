...
export default function RolePilotApp() {
  ...
  async function refreshFeed(mergeAfterLoad = false) {
    try {
      const loadStaticFeed = async () => {
        const staticRes = await fetch(`/gcc_product_feed.js?t=${Date.now()}`, { cache: "no-store" });
        const source = await staticRes.text();
        const json = source
          .replace(/^\s*window\.GCC_PRODUCT_JOB_FEED\s*=\s*/, "")
          .replace(/;\s*$/, "");
        return JSON.parse(json) as AutomationFeed & { error?: string };
      };

      let nextFeed: AutomationFeed & { error?: string };
      try {
        const res = await fetch(`/api/gcc-feed?t=${Date.now()}`, { cache: "no-store" });
        nextFeed = await res.json() as AutomationFeed & { error?: string };
        if (nextFeed.error || !nextFeed.generatedAt) nextFeed = await loadStaticFeed();
      } catch {
        nextFeed = await loadStaticFeed();
      }
    ...
    <main className="app-shell">
      <aside className="sidebar">
        <div className="brand brand-sidebar">
          ...
          <div><h1>RolePilot</h1><div className="brand-version">Career OS</div></div>
        </div>
        ...
        <div className="sidebar-foot">
          <div className="user-mark">{initials(user.name)}</div>
          <div className="sidebar-user"><strong>{user.name}</strong><span>{user.plan} plan</span></div>
          <button className="icon-btn" title="Sign out" onClick={() => { clearUser(); setUser(null); void signOutCloudUser(); }}><LogOut size={17} /></button>
        </div>
      </aside>
      ...
      <section className="workspace">
        <header className="topbar">
          <div className="page-heading">
            <span className="eyebrow">RolePilot / {view}</span>
            <h2>{view}</h2>
          </div>
          <div className="top-actions">
            <button className="icon-btn" title="Refresh job feed" onClick={() => void refreshFeed(false)}><Bell size={18} /></button>
            <a className="btn" href={viewUrl("Match Engine")} onClick={e => { e.preventDefault(); setActiveView("Match Engine"); }}><Sparkles size={16} /> Match</a>
            <button className="btn primary" disabled={!canAddJob} onClick={() => setEditing(newJob({ priority: jobs.length + 1 }))}>Add job</button>
          </div>
        </header>

        <div className="mobile-nav">
          {[...primaryViews, ...secondaryViews].map(item => (
            <a key={item} className={`tab ${view === item ? "active" : ""}`} href={viewUrl(item)} onClick={e => { e.preventDefault(); setActiveView(item); }}>{item}</a>
          ))}
        </div>
      ...
      <section className="panel command-center">
        <div className="command-main">
          ...
          <h2 className="command-title">
            {nextJob ? <><span>{nextJob.company}</span><span className="command-role">{nextJob.role}</span></> : "Build today's application queue"}
          </h2>
          <p>{nextAction}</p>
  ...
  return (
    <section className="panel cloud-panel">
      ...
      <div className="panel-body stack">
        ...
        <div className="copy-box"><b>Setup files</b><br />Run <code>supabase/schema.sql</code> in Supabase SQL editor, then add <code>NEXT_PUBLIC_SUPABASE_URL</code> and <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> to Cloudflare Pages and local env.</div>
      </div>
    </section>
  );
}
